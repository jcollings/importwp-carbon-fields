...
git clone --branch $BRANCH git@github.com:jcollings/importwp-carbon-fields.git $FOLDER
cd $FOLDER
git rm -rf .
...
sed -i -e "s/__STABLE_TAG__/$TAG/g" carbon-fields.php

# Confirm pushing of build.
while true; do

read -p "Do you want to push build ImportWP Carbon Fields v$TAG? (y/n) " yn
